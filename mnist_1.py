...
#利用变量管理重写辅助函数
def inference(input_tensor, avg_class, _reuse=False):
    if avg_class==None:
        #定义第一层神经网络的变量和前向传播过程
        with tf.variable_scope('layer1', reuse=_reuse):
            #根据传进来的reuse判断是创建新变量还是使用已经创建好的。在第一次构建网络时需要创建，以后每次调用时使用reuse=True
            weights = tf.get_variable("weights", [INPUT_NODE, LAYER1_NODE], initializer=tf.truncated_normal_initializer(stddev=0.1))
            biases = tf.get_variable("biases", [LAYER1_NODE], initializer=tf.constant_initializer(0.0))
            layer1 = tf.nn.relu(tf.matmul(input_tensor, weights) + biases)
        with tf.variable_scope('layer2', reuse=_reuse):
        ...
        #之前放在外面，因为所有参数直接被tf.Variable定义，现在没有，会报错
        return layer2
